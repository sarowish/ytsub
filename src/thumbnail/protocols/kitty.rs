...
pub fn display_image(image: DynamicImage) -> Result<()> {
    ...
    let mut it = bytes.chunks(4096).peekable();
    let mut buf = String::with_capacity(bytes.len() + it.len() * 60);

    if let Some(first) = it.next() {
        write!(
            buf,
            "{}_Gq=2,i={},a=T,U=1,f=24,t=d,s={width},v={height},m={};{}{}\\{}",
            *START,
            *IMAGE_ID,
            u8::from(it.peek().is_some()),
