...
fn handle_key_normal_mode(key: KeyEvent, app: &mut App) -> bool {
    if let Some(command) = KEY_BINDINGS.get(&key) {
        match command {
            ...
            Command::ToggleAudio => app.toggle_audio(),
            Command::SeekAudioBackward => app.seek_audio(-10),
            Command::SeekAudioForward => app.seek_audio(10),
            Command::IncreaseAudioVolume => app.adjust_audio_volume(2),
            Command::DecreaseAudioVolume => app.adjust_audio_volume(-2),
            Command::ToggleAudioMute => app.toggle_audio_mute(),
            Command::StopAudio => app.stop_audio(),
