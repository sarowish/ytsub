...
fn handle_key_normal_mode(key: KeyEvent, app: &mut App) -> bool {
    if let Some(command) = KEY_BINDINGS.get(&key) {
        match command {
            ...
            Command::PlayAudio => app.play_audio(),
            Command::PlayAudioUsingYtdlp => app.play_audio_using_ytdlp(),
            Command::TogglePlayback => app.toggle_playback(),
            Command::SeekPlaybackBackward => app.seek_playback(-10),
            Command::SeekPlaybackForward => app.seek_playback(10),
            Command::IncreaseVolume => app.adjust_volume(2),
            Command::DecreaseVolume => app.adjust_volume(-2),
            Command::ToggleMute => app.toggle_mute(),
            Command::StopPlayback => app.stop_playback(),
            Command::ReleaseVideo => app.release_video(),
